...
RegisterNetEvent('psr-phonograph:server:playMusic', function(song, zoneName)
    local src = source
    ...
    TriggerClientEvent('psr-phonograph:client:playMusic', src)
end)

RegisterNetEvent('psr-phonograph:server:stopMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
...
RegisterNetEvent('psr-phonograph:server:pauseMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[data.zoneName].coords
...
RegisterNetEvent('psr-phonograph:server:resumeMusic', function(data)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[data.zoneName].coords
...
RegisterNetEvent('psr-phonograph:server:changeVolume', function(volume, zoneName)
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    local boothCoords = Config.Locations[zoneName].coords
